...
app = FastAPI(title="LLM Interface")
# All subsystem routes are mounted by load_plugins() during startup.
...
async def startup():
    ...
    # Load plugins (tasks, etc.) — handles db_init, routes, tools, services
    ...
    from llm_interface.email_ingestion import _DEFAULT_PARSE_PROMPT
...
async def plugin_nav():
    """Aggregate nav entries from all loaded plugins."""
    entries = []
    for p in loaded_plugins:
        manifest = p.frontend_manifest()
        if manifest and manifest.nav_entries:
...
async def get_conversation(conversation_id: str):
    ...
    if type_row:
        ...
        if type_row["metadata"]:
            from llm_interface.backrooms import _normalize_metadata
...
def main():
    ...
    print(f"Starting LLM Interface on http://localhost:{port}")
    uvicorn.run(
        "llm_interface.server:app",
        host="0.0.0.0",
        port=port,
        reload=True,
    )
