...
| `calendar_db.py` | `CalendarDatabase` — SQLite cache for Google Calendar events. Upsert by google_event_id, query by time range |
| `calendar_routes.py` | FastAPI `APIRouter` for `/api/calendar/*` — OAuth2 flow (auth/callback), event fetching with cache, event creation |
| `calendar_tools.py` | Claude tool definitions for calendar (calendar_today, calendar_week, calendar_create) |
