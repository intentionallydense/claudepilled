...
- **Daily briefing assembly** — the `briefing` pip package handles all data gathering (RSS feeds, reading lists, Wikipedia, Anki) and rendering (LLM via OpenRouter or plain template). The wrapper imports and calls `briefing.assembly.assemble_briefing()` directly (no subprocess), then stores the result in its own DB for the UI. Reading progress is proxied from the standalone DB at `~/.briefing/state.db`. Anki stats re-exported from `briefing.anki`. Idempotent: won't re-assemble if today's briefing exists (unless forced). Can run via cron CLI (`llm-interface-briefing`) or the web UI's "assemble" button.
