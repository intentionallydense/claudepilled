...
| `style.css` | Shared layout styles — 4-column grid (`.app` default) and 2-column variant (`.app-simple` for settings/tasks), sidebar, messages, input area, tree panel, board panel, backrooms-specific styles, shared form components (`.form-label`, `.form-select`, `.form-textarea`, `.form-input`, `.form-actions`, `.btn-dark`). Includes `@media (max-width: 768px)` responsive block for mobile |
...
- **Shared layout system** — all pages load `style.css` for shared layout classes and form components; page-specific CSS files add only overrides. Chat, briefing, and backrooms use the 4-column grid: `sidebar (200px) | chat (max 700px) | tree (200px) | content (1fr)`. Settings and tasks use the `.app-simple` 2-column variant: `sidebar (200px) | scrollable content (1fr, max 700px)`. Both variants share the same sidebar component and mobile responsive behavior. `.btn-dark` is scoped: base is compact (for page buttons), `.modal-body .btn-dark` gets full-width (for modal actions).
