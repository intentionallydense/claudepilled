...
| `briefing/` | Daily briefing delivery — reads pre-assembled briefings from standalone project (`~/.briefing/briefings/*.md`). Provides `briefing_db`. Contains `db.py` (BriefingDatabase — briefings table), `anki.py` (AnkiConnect proxy), `assembly.py` (reads standalone .md files, shells out to `briefing assemble` if missing, CLI entry point), `routes.py` (3 sub-routers: briefing CRUD, reading-progress proxy to `~/.briefing/state.db`, anki stats). Combined router mounted at `/api` |
...
| File | Purpose |
|------|---------|
| `sequences_order.json` | LessWrong Rationality: A-Z reading order (~338 posts). Legacy — reading lists now managed by standalone briefing project |
| `gwern_essays.json` | Curated Gwern.net essays sorted by importance (~80 essays). Legacy — reading lists now managed by standalone briefing project |
| `acx_best_of.json` | SSC + ACX best-of compilation (~126 posts). Legacy — reading lists now managed by standalone briefing project |
| `album_list.json` | /mu/ essential albums (~500 albums). Legacy — reading lists now managed by standalone briefing project |
...
- **Daily briefing assembly (standalone)** — briefing content is assembled by a standalone project that writes markdown files to `~/.briefing/briefings/YYYY-MM-DD.md`. This wrapper reads those files and stores them in its own DB for the UI. If today's file doesn't exist, the assemble endpoint shells out to `briefing assemble`. Reading progress is proxied from the standalone project's DB at `~/.briefing/state.db`. Idempotent: won't re-import if today's briefing exists (unless forced). Can run via cron CLI (`llm-interface-briefing`) or the web UI's "assemble" button.
...
- **System model routing** — cheap system tasks (title generation, compaction summaries, email parsing) use GLM5 via OpenRouter (`z-ai/glm-5`) with reasoning/thinking enabled for richer output, falling back to Anthropic Haiku if no OpenRouter key is configured. Brain dump (model-speaks-first) uses GLM5 via OpenRouter (has tool use for task creation). Briefing assembly is handled by the standalone briefing project; briefing chat conversations use the user's selected model (defaults to Sonnet 4.6 via settings).
