...
Claude Wrapper is a personal web UI and API wrapper supporting multiple LLM providers — Anthropic Claude, OpenAI GPT, Google Gemini, DeepSeek, Qwen, and Kimi. It provides streaming chat with tool use (Anthropic), an N-model "backrooms" conversation mode (2-5 participants, cross-provider, with AI self-modification commands), conversation branching, file context injection via tags, and a task management system — all backed by SQLite and served via FastAPI.
...
| `backrooms.py` | `BackroomsOrchestrator` — N-model (2-5) chatroom with round-robin turns, role-flipping, [ready] signaling, command parsing, speed control, stats tracking. Metadata v2 uses `participants` array; v1 (model_a/model_b) auto-normalized on read. `_resolve_prompt()` uses seat indices with `{self_label}/{other_label}/{all_labels}/{seat_number}` substitution. Integrates with `backrooms_commands.py` for AI self-modification |
| `backrooms_commands.py` | Command parser and executor for backrooms AI self-modification. Regex-based extraction of `!prompt`, `!temperature`, `!whisper`, `!mute_self`, `!vote`, `!search`, `!image` from model output. `SessionState` holds mutable per-session state. Commands stripped from visible text; results appear as system notifications |
...
| File | Purpose |
|------|---------|
| `index.html` | Main page — 4-column layout: sidebar, chat, tree, moodboard. Hosts both chat and backrooms sessions (unified). Includes PWA manifest, hamburger button for mobile, backrooms modals (new session, prompts) |
...
| `backrooms-adapter.js` | Adapter for N-model backrooms sessions — handles backrooms-specific streaming events (`backrooms_turn_start/end`, `backrooms_paused`, `backrooms_status`, `backrooms_command`, `backrooms_stats`), speaker-labeled message rendering (model_0 through model_4), edit/regenerate for curator messages, dynamic per-seat prompts modal, speed control buttons (0.5x/1x/2x/5x), stats display, whisper rendering. Provides `getChatCoreConfig()` overrides for chat-core. Used by `app.js` when a backrooms session is opened |
| `app.js` | Main page logic — sidebar (unified: both chat and backrooms sessions), moodboard, tag autocomplete, context bar, mobile sidebar toggle. Supports mode switching: destroys/recreates chatCore with appropriate config when switching between chat and backrooms sessions. "backrooms" link opens new-session modal |
| `style.css` | Shared layout styles — 4-column grid, sidebar, messages, input area, tree panel, board panel, backrooms-specific styles (speaker dimming, model names, prompts modal). Includes `@media (max-width: 768px)` responsive block for mobile |
...
| `settings.html/js/css` | Settings page — model defaults, universal prompt, prompt library (chat + backrooms categories), per-seat backrooms suffixes |
...
- **Streaming via WebSocket** — the chat and backrooms pages use persistent WebSocket connections. Events are typed (`StreamEvent`) and serialized as JSON.
- **Backrooms N-participant system** — supports 2-5 models in round-robin turns with role-flipping. Metadata v2 stores a `participants` array (`[{seat, id, label, speaker}, ...]`); old v1 format (model_a/model_b) is auto-normalized on read via `_normalize_metadata()`. Speaker IDs are `model_0` through `model_4`. Each model sees others' messages as "user" role with label prefix. Messages are merged to ensure strict role alternation. The backrooms WebSocket supports `edit`, `regenerate`, `inject_tags`, and `set_speed`. Prompts resolved by seat index: `prompt_ids[seat]` → legacy `prompt_id_a/b` → `system_prompt_a/b` → default template. Variables: `{self_label}`, `{other_label}` (comma-joined others), `{all_labels}`, `{seat_number}`. Per-seat suffixes (`backrooms_seat_N_suffix`, N=1-5). Seat 0 manages pacing via [ready] signaling.
- **Backrooms command system** — after each model response, `parse_commands()` extracts `!command` patterns via regex. Commands: `!prompt "text"` (append to own prompt), `!temperature X` (0.0-2.0), `!whisper "target" "msg"` (private, only visible to source/target during message building), `!mute_self` (skip next turn), `!vote "q" [opts]` (informational poll), `!search "query"` (Anthropic web search via Haiku one-shot), `!image "desc"` (DALL-E 3). Commands stripped from visible text; results injected as system notification messages. `SessionState` holds per-stream-call mutable state (prompt appendices, temperatures, mute counters, speed). Search and image fail gracefully without API keys.
- **Backrooms speed control** — curator can set turn delay multiplier (0.5x/1x/2x/5x) via WebSocket action. Speed stored in session metadata and applied to `asyncio.sleep()` between turns. Frontend renders small inline buttons next to status indicator.
...
- **Unified 4-column layout** — chat, briefing, and backrooms all share the same page and CSS grid: `sidebar (200px) | chat (max 700px) | tree (200px) | content (1fr)`. Tree is adjacent to chat for quick branch navigation. Column 4 is the moodboard on the chat page and briefing content on the briefing page. All pages load `style.css` for shared layout classes; page-specific CSS files add only overrides.
