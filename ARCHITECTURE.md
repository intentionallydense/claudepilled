...
| `backrooms.py` | `BackroomsOrchestrator` — N-model (2-5) chatroom with iteration-based pacing, round-robin turns, role-flipping, command parsing, speed control, stats tracking. Pacing: curator sets `iterations` (how many full rounds auto-run, default 1), `step_mode` (one turn at a time), `next_speaker` (one-shot override). `_pick_next_seat()` is the extension point for custom orchestration. Metadata v2 uses `participants` array; v1 (model_a/model_b) auto-normalized on read. `_resolve_prompt()` uses seat indices with `{self_label}/{other_label}/{all_labels}/{seat_number}` substitution. Integrates with `backrooms_commands.py` for AI self-modification |
