...
| `briefing/` | Daily briefing — integrates the standalone `briefing` package (pip dependency). Provides `briefing_db`. Contains `db.py` (WrapperBriefingDB — briefings table for UI), `anki.py` (re-exports from `briefing.anki`), `assembly.py` (calls `briefing.assembly.assemble_briefing()` Python API, stores result in wrapper DB, CLI entry point), `routes.py` (3 sub-routers: briefing CRUD, reading-progress proxy to `~/.briefing/state.db`, anki stats). Combined router mounted at `/api` |
...
- **Daily briefing assembly** — the `briefing` pip package handles all data gathering (RSS feeds, reading lists, Wikipedia, Anki) and rendering (LLM via OpenRouter or plain template). The wrapper imports and calls `briefing.assembly.assemble_briefing()` directly (no subprocess), seeds reading progress via `init_all_series()` before each assembly, then stores the result (including gathered sections data) in its own DB for the UI. Reading progress is proxied from the standalone DB at `~/.briefing/state.db`. Anki stats re-exported from `briefing.anki`. Idempotent: won't re-assemble if today's briefing exists (unless forced). Runs automatically via the plugin cron system (every 30 min, with run-on-start), via cron CLI (`llm-interface-briefing`), or the web UI's "assemble" button.
- **Plugin cron jobs** — `plugin_loader.py` collects `cron_jobs()` from all plugins after loading and starts asyncio background tasks. Each job spec has `{name, fn, interval_seconds, run_on_start}`. Sync functions run in the thread executor to avoid blocking the event loop. Jobs are fire-and-forget with exception logging. The briefing plugin uses this to auto-assemble daily briefings on server startup and every 30 minutes thereafter.
...
- **System model routing** — cheap system tasks (title generation, compaction summaries, email parsing) use GLM5 via OpenRouter (`z-ai/glm-5`) with reasoning/thinking enabled for richer output, falling back to Anthropic Haiku if no OpenRouter key is configured. Brain dump (model-speaks-first) uses GLM5 via OpenRouter (has tool use for task creation). Briefing assembly uses the `briefing` package (GLM5/template); briefing chat conversations use the user's selected model (defaults to Sonnet 4.6 via settings).
